...
auto ReconstructTuple(const Schema *schema, const Tuple &base_tuple, const TupleMeta &base_meta,
  ...
  for (const auto& undo : undo_logs) {
    ...
    auto undo_tuple_schema = Schema::CopySchema(schema, std::move(attrs));
...
void TxnMgrDbg(const std::string &info, TransactionManager *txn_mgr, const TableInfo *table_info,
  ...
  */
  auto table_iter_ = table_heap->MakeIterator();
  while (!table_iter_.IsEnd()) {
    ...
    if ((meta.ts_ & mask) != 0) {
      fmt::println(stderr, "RID={}/{} ts=txn{} tuple={}", rid.GetPageId(), rid.GetSlotNum(), meta.ts_^mask, tuple.ToString(&table_info->schema_));
    } else {
      fmt::println(stderr, "RID={}/{} ts={} tuple={}", rid.GetPageId(), rid.GetSlotNum(), meta.ts_, tuple.ToString(&table_info->schema_));
    }
    ...
    while (undo_link.has_value() && undo_link->IsValid()) {
      ...
      if (undo_log.is_deleted_) {
        fmt::println(stderr, "txn{}@{} {} ts={}", undo_link->prev_txn_^mask, undo_link->prev_log_idx_, "<Del>", undo_log.ts_);
      } else {
        std::vector<uint32_t> attrs;
        ...
        auto schema = Schema::CopySchema(&table_info->schema_, std::move(attrs));
        fmt::println(stderr, "txn{}@{} {} ts={}", undo_link->prev_txn_^mask, undo_link->prev_log_idx_, undo_log.tuple_.ToString(&schema), undo_log.ts_);
      }
