...
void NestedLoopJoinExecutor::Init() {
  ...
  while (left_executor_->Next(&left_tuple, &left_rid)) {
    // Assertion `(casted_right_executor->GetInitCount() + 1 >= casted_left_executor->GetNextCount()) && ("nlj check failed, are you initialising the right executor every time when there is a left tuple? " "(off-by-one is okay)")' failed.
    right_executor_->Init();
    bool match = false;
