...
void WindowFunctionExecutor::Init() {
    ...
    for (const auto& [_, wf] : plan_->window_functions_) {
        if (!wf.order_by_.empty()) {
            std::function<bool(const std::pair<Tuple, RID>&, const std::pair<Tuple, RID>)>
            kp = [&wf = wf, &child_executor_ = child_executor_](const std::pair<Tuple, RID>& a, const std::pair<Tuple, RID>& b) -> bool {
            ...
            switch (wf.type_) {
                ...
                case WindowFunctionType::Rank:
                    ...
                    if (!input.IsNull()) {
                        ...
                        pre_input[key].first = std::move(input);
                    }
                    break;
            }
            if (!wf.order_by_.empty()) {
    ...
    for (const auto& [tuple, rid] : child_tuple) {
        ...
        for (const auto& expr : plan_->columns_) {
            ...
            if (column_expr->GetColIdx() == (uint32_t)-1) {
