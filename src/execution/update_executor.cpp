...
auto UpdateExecutor::Next([[maybe_unused]] Tuple *tuple, RID *rid) -> bool {
  ...
  if (!is_done_) {
		...
		while (child_executor_->Next(&child_tuple, &child_rid)) {
			...
			auto ret = table->InsertTuple({0, false}, new_tuple, exec_ctx_->GetLockManager(), exec_ctx_->GetTransaction(), plan_->GetTableOid());
			if (ret != std::nullopt) {
				num++;
				...
				for (auto& idx_info : table_indexes) {
					...
					index->DeleteEntry(child_tuple.KeyFromTuple(table_info->schema_, idx_info->key_schema_, index->GetKeyAttrs()), child_rid, exec_ctx_->GetTransaction());
					index->InsertEntry(new_tuple.KeyFromTuple(table_info->schema_, idx_info->key_schema_, index->GetKeyAttrs()), *ret, exec_ctx_->GetTransaction());
				}
			}
		}
